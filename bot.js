...
const getInlineKeyboard = (currentPage, totalPages, isSelected) => {
  ...
  if (isSelected) {
    ...
    if (currentPage > 0) {
      buttons.push(Markup.button.callback("⬅️ Backward", `backward:${currentPage}`));
    }

    buttons.push(Markup.button.callback(`🎧 Select (${currentPage + 1}/${totalPages + 1})`, `select:${currentPage}`));

    if (currentPage < totalPages) {
      buttons.push(Markup.button.callback("➡️ Forward", `forward:${currentPage}`));
    }
  }
...
const updateMessage = async (ctx, searchResults, currentPage, isSelected = false) => {
  ...
  await ctx.editMessageText(
    ...
    {
      parse_mode: "HTML",
      reply_markup: getInlineKeyboard(currentPage, searchResults.length - 1, isSelected).reply_markup,
    }
  );
};
...
bot.command("search", async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      ...
      {
        parse_mode: "HTML",
        reply_markup: getInlineKeyboard(currentPage, searchResults.length - 1, false).reply_markup,
      }
    );
  } catch (error) {
    console.error(error);
...
bot.action(/^(forward|backward|select):(\d+)$/, async (ctx) => {
  ...
  if (action === "select") {
    ...
    try {
      ...
      await ctx.telegram.sendChatAction(ctx.callbackQuery.from.id, "upload_audio");

      await ctx.telegram.sendAudio(
        ctx.callbackQuery.from.id,
        { url: downloadUrl, filename: `${artistName} - ${songName}.mp3`, thumbnail: { url: albumArt, filename: "albumArt.jpg"}},
        {
          title: songName,
...
bot.command("download", async (ctx) => {
  ...
  if (!spotifyUrl.startsWith("https://open.spotify.com/track/")) {
    const errorMessage = await ctx.reply("❌ Please enter a valid Spotify song URL, starting with 'https://open.spotify.com/track/'.\n\n🗑️ This message will be automatically deleted in 10 seconds...");
